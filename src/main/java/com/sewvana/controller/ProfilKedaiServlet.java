...
@WebServlet("/penjahit/profil-kedai")
public class ProfilKedaiServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        request.getRequestDispatcher("/views/penjahit/profil_kedai.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("tukar_password".equals(action)) {
            ...
            // Default: Kemaskini Profil Kedai (Nama & Telefon)
            String nama = request.getParameter("nama");
            String telefon = request.getParameter("telefon");

            if (nama == null || nama.trim().isEmpty()) {
                errorMsg = "Nama kedai/penjahit tidak boleh dibiarkan kosong.";
            } else {
                try {
                    boolean success = penggunaDAO.kemaskiniProfil(user.getId(), nama.trim(), telefon != null ? telefon.trim() : "");
                    if (success) {
                        successMsg = "Maklumat profil kedai anda berjaya dikemas kini.";
                        user.setNama(nama.trim());
                        user.setTelefon(telefon != null ? telefon.trim() : "");
                        session.setAttribute("pengguna", user);
                    } else {
                        errorMsg = "Gagal mengemas kini profil kedai ke database.";
                    }
                } catch (SQLException e) {
                    e.printStackTrace();
                    errorMsg = "Ralat sistem: " + e.getMessage();
